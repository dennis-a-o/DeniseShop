package com.example.deniseshop.ui.screens.flashsale
...
class FlashSaleViewModel @Inject constructor(
	...
	private val _flashSale = MutableStateFlow<UiFlashSale?>(null)
	...
	private fun getFlashSale(){
		getFlashSaleUseCase(flashSaleId).onEach {
			when(it){
				is NetworkResponseState.Error -> {}
				is  NetworkResponseState.Loading -> {}
				is NetworkResponseState.Success -> {
					_flashSale.value = flashSaleApiToUiMapper.map(it.result)
				}
			}
		}.launchIn(viewModelScope)
	}
