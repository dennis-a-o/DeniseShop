...
import com.example.deniseshop.feature.categoryproducts.components.CategoriesSection
...
fun CategoryProductsScreen(
	...
	val productItems = viewModel.categoryProductsPagingSource.collectAsLazyPagingItems()
	...
	if (showProductFilterBottomSheet){
		ProductFilterBottomSheet(
			...
			onApplyFilterState = {
				viewModel.onFilterStateChange(it)
				...
				is LoadState.NotLoading -> {
					Column(
						...
						LazyVerticalGrid(
							...
							state.category?.categories?.let { categories ->
								item (span = { GridItemSpan(if (isGrid) 2 else 1) }){
									CategoriesSection(
										categories = categories,
										onCategoryClick = { id ->
											onNavigate("${Routes.CategoryProductScreen.route}/$id")
										}
									)
								}
							}

							state.category?.brands?.let { brands ->
								item (span = { GridItemSpan(if (isGrid) 2 else 1) }){
									BrandsSection(
										brands = brands,
										onBrandClick = { id ->
